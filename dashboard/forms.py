...
from dashboard.models import Umuryango, Cell, KPI, Village, District, Sector
...
class FamilyForm(forms.ModelForm):
    ...
    class Meta:
        ...
        # --------------snippets for loading sector based on district -------------#
        def __init__(self, *args, **kwargs):
            super().__init__(*args, **kwargs)
            self.fields['sector'].queryset = District.objects.none()

            if 'district' in self.data:
                try:
                    district_id = int(self.data.get('district'))
                    self.fields['sector'].queryset = Sector.objects.filter(district_id=district_id).order_by('name')

                except (ValueError, TypeError):
                    pass

            elif self.instance.pk:
                self.fields['sector'].querset = self.instance.district.sector_set.order_by('name')

    # -------------------------------------------------------------------------end sector based on dist -------------#
    ...
    # --------------snippets for loading village based on cell -------------#
    def __init__(self, *args, **kwargs):
        super().__init__(*args, **kwargs)
